...
class QueryRouter:
    ...
    def classify(
        self, query: str, k8sgpt_results: Optional[List] = None
    ) -> EnrichmentPlan:
        """
        ...
        """
        # Validate query first and clean backticks
        is_valid, error, cleaned_query = self.sanitizer.validate_query(query)
        if not is_valid:
        ...
        scores = {
            QueryCategory.POD_ISSUE: self._count_keywords(
                query_lower, self.POD_KEYWORDS
            ),
            QueryCategory.DEPLOYMENT_STATUS: self._count_keywords(
                query_lower, self.DEPLOYMENT_KEYWORDS
            ),
            QueryCategory.SERVICE_NETWORKING: self._count_keywords(
                query_lower, self.SERVICE_NETWORKING_KEYWORDS
            ),
            QueryCategory.NODE_HEALTH: self._count_keywords(
                query_lower, self.NODE_KEYWORDS
            ),
            QueryCategory.STORAGE: self._count_keywords(
                query_lower, self.STORAGE_KEYWORDS
            ),
            QueryCategory.ARGOCD: self._count_keywords(
                query_lower, self.ARGOCD_KEYWORDS
            ),
            QueryCategory.SECURITY: self._count_keywords(
                query_lower, self.SECURITY_KEYWORDS
            ),
        }
        ...
        logger.info(
            f"Classified query into categories: {[c.value for c in categories]}"
        )
