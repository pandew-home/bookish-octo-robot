...
class InputSanitizer:
    ...
    _compiled_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in UNSAFE_PATTERNS]
    ...
    def validate_query(self, query: str) -> Tuple[bool, Optional[str]]:
        ...
        if len(query) > 2000:
            return False, f"Query is too long ({len(query)} characters). Please limit your query to 2000 characters."
    ...
    def _get_helpful_rejection_message(self, pattern: str) -> str:
        ...
        if 'passwd' in pattern or 'shadow' in pattern or 'credentials' in pattern or 'AKIA' in pattern:
            return (
    ...
    def validate_aws_credentials(
        ...
        if not re.match(r'^AKIA[0-9A-Z]{16}$', access_key):
            return False, "Invalid access key format. AWS access keys start with 'AKIA' followed by 16 characters."
        
        # Validate secret key length (40 characters)
        if len(secret_key) != 40:
            return False, f"Invalid secret key length. AWS secret keys are exactly 40 characters (got {len(secret_key)})."
        
        # Validate session token (should be substantial length)
        if len(session_token) < 16:
            return False, "Invalid session token. Session tokens should be at least 16 characters."
